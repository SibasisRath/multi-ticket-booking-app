...
public class BookingsController {
	...
	@PostMapping(path = "/bus/bookbustickets", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed(value = "book_bus_tickets")
	public ResponseEntity<String> bookTicket(@Valid @RequestBody BookingsDto dto) {
		log.info("inside book ticket controller.");
		return ResponseEntity.ok(busBookingsService.bookingTickets(dto));
	}

	@GetMapping(path = "/bus/bookinglist/{userid}/{dateFrom}")
	@Timed(value = "get_bus_booking_details_by_userId_datefrom")
	public Object getBookingDetails(@PathVariable("userid") String userId, @PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate dateFrom) {
	...
	@GetMapping(path = "/bus/bookinglist/{userId}")
